...
public final class FlyCommand implements CommandExecutor, TabCompleter {
    ...
    public FlyCommand(PlayerModeService service) {
        this.service = service;
    }

    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!sender.hasPermission("auriongo.command.misc.fly")) {
            sender.sendMessage(this.service.renderNoPermission());
            return true;
        }

        if (args.length > 1) {
            sender.sendMessage(this.service.renderFlyUsage());
            return true;
        }
        ...
        this.service.applyFly(sender, target);
        return true;
    }
